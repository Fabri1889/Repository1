...
form.addEventListener('submit', (e) => {
  ...
  if (errors.length === 0) {
    ...
    const url = 'http://curso-dev-2021.herokuapp.com/newsletter?' + new URLSearchParams(data);

    fetch(url)
      .then(response => response.json())
      .then(responseData => {
        localStorage.setItem('newsletterData', JSON.stringify(responseData));

        loadFormData();

        const modalMessage = document.getElementById('modal-message');
        modalMessage.textContent = 'Suscripción exitosa:\n' + JSON.stringify(responseData);
      ...
      .catch(error => {
        const modalMessage = document.getElementById('modal-message');
        modalMessage.textContent = 'Error en la suscripción:\n' + error.message;
